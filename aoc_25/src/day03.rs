...
fn part1(input: &str) -> u32 {
    ...
    for line in lines{
        let joltages: Vec<u32> = line.chars().into_iter().map(|x| x.to_digit(10).unwrap()).collect();
        let mut max_joltages = vec![0,0];
        let n = joltages.len();
        for i in 0..n{
            let joltage = joltages[i];
            if i < (n-1) && joltage > max_joltages[0]  {
                max_joltages[0] = joltage;
                max_joltages[1] = 0;
            }else if joltage > max_joltages[1]{
                max_joltages[1] = joltage
            };
        }
